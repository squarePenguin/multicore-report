...
public:
  void acquire_write() {
    int zero = 0;
    do {
      if (lock.load() == 0 && lock.compare_exchange_weak(zero, -1, memory_order_acquire, memory_order_relaxed)) {
        break;
      }
    } while (true);
  }
  ...
  void acquire_read() {
    do {
      int old_val = lock.load(memory_order_relaxed);
      if (old_val >= 0 && lock.compare_exchange_weak(old_val, old_val + 1, memory_order_acquire, memory_order_relaxed)) {
        break;
      }
    } while (true);
  }
  ...
  void release_read() {
    lock.fetch_add(-1, memory_order_release);
  }
};
...
mutex sum_mutex;
Tatas_lock sum_tatas;
Rw_lock sum_rw;

static __attribute__ ((noinline)) int __attribute__((optimize("O0"))) 
sum(int arr[], int size, Lock_type lock=Lock_type::no_lock) {
  if (lock == Lock_type::mutex) {
    sum_mutex.lock();
  } else if (lock == Lock_type::tatas) {
    sum_tatas.lock();
  } else if (lock == Lock_type::rw) {
    sum_rw.acquire_read();
  }
...
void read_only_test(int num_threads, int arr_size, int num_loops, Lock_type lock=Lock_type::no_lock) {
  ...
  workers.emplace_back([=, &done, &arr]() {
    int64_t s = 0;
    for (int i = 0; i < num_loops; i += 1) {
      s += sum(arr.data(), arr.size(), lock);
    }
    done.store(true);
    cout << "s=" << s << endl;
  });
...
int main(int argc, char** argv) {
  ...
  if (test == Test::SPIN) {
    ...
    read_only_test(num_threads, arr_size, delay, lock_type);
  }
  ...
  // int one = 1;
  // int two = 2;
  // v = x.compare_exchange_strong(zero, one);
  // cout << "x=" << x << " v=" << v << endl;
  // v = x.compare_exchange_strong(zero, two);
  // cout << "x=" << x << " v=" << v << endl;
  // v = x.compare_exchange_strong(one, two);
  // cout << "x=" << x << " v=" << v << endl;
